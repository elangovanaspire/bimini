...
include_once( dirname( __FILE__ ) . '/lib/class-crocs-json-posts.php' );
...
function json_api_default_filters( $server ) {
	...
	add_filter( 'json_prepare_taxonomy', array( $wp_json_posts, 'add_post_type_data' ), 10, 3 );
        ...
        $crocs_json_posts = new CROCS_JSON_Posts( $server );
	add_filter( 'json_endpoints', array( $crocs_json_posts, 'register_routes' ), 0 );
	add_filter( 'json_prepare_taxonomy', array( $crocs_json_posts, 'add_post_type_data' ), 10, 3 );
